...
if not os.path.exists("models/experiment_02"):
    os.makedirs("models/experiment_02")
if not os.path.exists("figures/experiment_02"):
    os.makedirs("figures/experiment_02")
...
for epoch_index in epoch_progress_bar:
    ...
    if epoch_index % EPOCH_SAVE_PERIOD == 0:
        with open(
            f"models/experiment_02/augmentation_model.chk_{epoch_index:05d}", "wb"
        ) as f:
            torch.save(velocity_field_model.state_dict(), f)
        with open("models/experiment_02/history.csv", "w") as f:
            df = pd.DataFrame(history)
            df.to_csv(f, index=False)
    if epoch_index % EPOCH_GENERATE_PERIOD == 0:
        create_and_save_images(
            f"figures/experiment_02/augmentation_{epoch_index:05d}.png"
        )


with open("models/experiment_02/augmentation_model.model", "wb") as f:
    torch.save(velocity_field_model.state_dict(), f)
with open("models/experiment_02/history.csv", "w") as f:
    df = pd.DataFrame(history)
    df.to_csv(f, index=False)
